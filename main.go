...
func main() {
	...
	if err != nil {
		slog.Error("Failed to get Change IDs for the stack", "err", err)
		os.Exit(1)
	}
	...
	for _, changeID := range changeIDs {
		...
		if prNum == -1 {
			...
			if lastBranch == "" {
				baseBranch = k.String("mainBranch")
			} else {
...
func getStackChangeIDs() ([]string, error) {
	out, err := Run(exec.Command(
		...
		"-r", fmt.Sprintf("%s..@-", k.String("mainBranch")),
		"-T", `change_id ++ "\n"`,
	))
	if err != nil {
		return []string{}, fmt.Errorf("output: %s, err: %v", out, err)
	}
